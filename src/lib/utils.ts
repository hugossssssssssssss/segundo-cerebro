...
export function formatarDataPtBR(iso?: string): string {
  ...
  const mIso = dataParte.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (mIso) {
    const ano = mIso[1].slice(2);
    const mes = mIso[2];
    const dia = mIso[3];
  ...
  // Caso 2: Formato DD/MM/YYYY ou DD/MM/YY
  const mBr = dataParte.match(/^(\d{1,2})\/(\d{1,2})\/(\d{2,4})/);
  if (mBr) {
    const dia = mBr[1].padStart(2, "0");
    const mes = mBr[2].padStart(2, "0");
    const ano = mBr[3].length === 4 ? mBr[3].slice(2) : mBr[3];
    const dataFormatada = `${dia}/${mes}/${ano}`;
