...
// Permite carregar os modelos abertos do HuggingFace via navegador
...
export async function obterTranscritorWhisperLocal(aoProgresso?: (msg: string) => void) {
  ...
  aoProgresso?.("Carregando modelo local do Whisper (OpenAI 100% navegador)...");

  // Modelo ultra leve e rápido (Xenova/whisper-tiny) para rodar suave no navegador
    ...
    "Xenova/whisper-tiny",
    {
      ...
      progress_callback: (p: any) => {
        if (p.status === "progress" && p.progress) {
          aoProgresso?.(`Baixando modelo local: ${Math.round(p.progress)}%`);
...
/**
 * Decodifica o áudio do usuário para 16kHz Mono (padrão esperado pelo Whisper)
 */
export async function decodificarAudioPara16kHz(file: File): Promise<Float32Array> {
  ...
  // Retorna os dados do canal 0 (mono)
  return audioBuffer.getChannelData(0);
}

/**
 * Transcreve o áudio 100% localmente no navegador sem nenhuma API externa ou chave.
 */
export async function transcreverAudioLocalWhisper(
  file: File,
  aoProgresso?: (msg: string) => void
): Promise<string> {
  aoProgresso?.("Preparando decodificação do áudio...");
  const audioData = await decodificarAudioPara16kHz(file);

  const transcritor = await obterTranscritorWhisperLocal(aoProgresso);

  aoProgresso?.("Processando transcrição 100% local (0ms custo)...");
...
function saibaComoFormatador(saida: any, nomeArquivo: string): string {
  ...
  if (typeof saida.text === "string" && saida.text.trim()) {
    ...
    if (Array.isArray(saida.chunks) && saida.chunks.length > 0) {
      ...
      saida.chunks.forEach((chunk: any, index: number) => {
        ...
        // Alterna sugestão de orador em grandes pausas
        if (index > 0 && chunk.timestamp?.[0] - (saida.chunks[index - 1].timestamp?.[1] || 0) > 2.5) {
          oradorAtual = (oradorAtual % 2) + 1;
        }

        md += `**[${min}:${seg}] Orador ${oradorAtual}**: ${chunk.text.trim()}\n\n`;
      });
    } else {
      md += `${saida.text.trim()}\n`;
    }
  ...
  return "Transcrição concluída sem texto legível.";
}
