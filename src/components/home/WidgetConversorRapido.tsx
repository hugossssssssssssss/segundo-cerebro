  ...
  return (
    <div className="flex flex-col justify-between h-full space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {conversoesPopulares.map((c, i) => (
          <button
            key={i}
            type="button"
            onClick={aoAbrirPopup}
            className="p-2.5 rounded-xl border border-border/60 bg-background/50 hover:bg-secondary/40 hover:border-border transition-colors text-left cursor-pointer"
          >
            <p className="text-[10px] text-muted-foreground font-medium">Converter</p>
